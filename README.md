...
3. **命令行唤醒（开发者）**：在终端运行标准命令 `npm run status`。
...
| **“查看 Token 占用状态”** / **“技能诊断”** | `npm run status` | 打印可视化的 Token 仪表盘与健康度汇报卡 |
...
**答**：支持两种方式：1. 对 AI 说“查看 Token 占用”、“技能诊断”；2. 在项目终端运行标准命令 `npm run status`。
...
| **手动唤醒** | 命令/语言唤醒诊断 | - [ ] 支持自然语言及 `npm run status` 随时唤醒 |
...
- **v2.0.0 (2026-07-28)**：全面实现 v2.0 工业级四大核心模块，统一规范为标准 `npm run status` 命令行手动唤醒。
