...
6. **5 秒网络超时熔断降级保护**：云端拉取遇到断网或超时，5 秒内自动秒切本地微模板兜底，保证对话永不卡死。
...
**答**：支持 5 大云端/本地注册表：1. Vercel (`skills.sh`)；2. Upskill 安全审计库 (`upskill.dev`)；3. GitHub 巨头 Org 仓库 (`stripe/agent-skills`)；4. 国内 Gitee/jsDelivr CDN 极速节点 (Ping <30ms)；5. 您的团队私有 GitHub 组织。
...
- **v2.2.5 (2026-07-28)**：扩充完善 Q&A 清单至 10 大核心直击痛点问题（涵盖开局 Token 优化、多源推断、5 大注册表去重防冲突、断网熔断机制等）。
- **v2.2.4 (2026-07-28)**：清理正文和标题中的装饰性 Emoji。
