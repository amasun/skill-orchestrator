...
2. **自然语言唤醒**：对 AI 说“查看 Token 占用”、“技能诊断”、“当前项目装了哪些技能”。
3. **命令行唤醒（开发者备用）**：在终端运行 `npm run telemetry` 或 `npm run status`。
...
## 🗣️ 自然语言与命令行触发指引 (Triggers & Commands)

| 自然语言表述（推荐） | 命令行对应指令（开发者） | 作用与效果 |
| :--- | :--- | :--- |
...
| **“查看 Token 占用状态”** / **“技能诊断”** | `npm run telemetry` / `npm run status` | 打印可视化的 Token 仪表盘与健康度汇报卡 |
...
**答**：支持两种方式：1. 对 AI 说“查看 Token 占用”、“技能诊断”；2. 在项目终端运行 `npm run telemetry` 或 `npm run status`。
...
| **手动唤醒** | 命令/语言唤醒诊断 | - [ ] 支持自然语言及 `npm run telemetry` / `npm run status` 随时唤醒 |
...
- **v2.0.0 (2026-07-28)**：全面实现 v2.0 工业级四大核心模块，支持 `npm run telemetry` / `npm run status` 命令行手动唤醒汇报卡。
