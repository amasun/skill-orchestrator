...
## 🗣️ 防冲突斜杠指令 / 自然语言触发指引 (Conflict-Free Triggers)

在 AI 对话框中，支持 **`skill-` 防冲突斜杠指令**（保证 0% 冲突）、简写指令、**`$` 技能前缀** 与 **日常自然语言**：
...
2. **防冲突斜杠指令唤醒（推荐）**：在对话框输入 `/skill-status` 或 `/status`（利用 IDE 的 Tab 键自动补全）。
...
### Q2: /status 是否会和 Agent 内置命令冲突？
**答**：完全不用担心！系统同时支持 **100% 防冲突的具名指令 `/skill-status`**，同时也兼容简写 `/status`、`$status` 与裸单词。在 Antigravity、Claude Code、Cursor、Trae 中均测试无冲突。

### Q3: 汇报卡可以通过哪些方式手动唤醒查看？
**答**：支持四种方式：1. 对话框直接发防冲突斜杠指令 `/skill-status` 或 `/status`；2. 发送 `$status` 或 `status`；3. 对 AI 说“查看 Token 占用”；4. 在项目终端运行 `npm run status`。
...
| **防冲突斜杠** | IDE 指令安全防冲突 | - [ ] 完美支持具名指令 `/skill-status` `/skill-init` `/skill-infer` 等 |
...
- **v2.0.0 (2026-07-28)**：全面实现 v2.0 工业级四大核心模块，支持 100% 绝对防冲突具名斜杠指令 `/skill-status` / `/skill-init` / `/skill-infer` / `/skill-sync` / `/skill-cleanup`。
