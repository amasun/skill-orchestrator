...
> **版本**：v2.0.0 (工业级全模块版)  
> **核心原则**：依赖自动推断 · 首轮自动透明汇报(含精确来源追溯) · Prompt 缓存锚点 · 熔断降级保障 · Token 预算实时诊断仪表盘  

---

## 📢 首轮对话自动汇报规范 (Proactive Telemetry Report Protocol)

在任何新项目/新工作区中，当需求定稿或首轮技能调配完成后，AI 会在回复末尾**自动精准标注技能来源与归属**，无需用户手动询问：
...
| **“检查 package.json 需要什么技能”** / **“自动匹配依赖技能”** | `npm run infer` | 自动扫描项目代码依赖，零沟通精准装载技能 |
...
| **可观察性与监控** | 无法感知技能占用的具体 Token 权重 | **Token Telemetry 诊断仪表盘** (可视健康度柱状输出 + 精确来源标注) |
...
flowchart TD
    ...
    subgraph LIFECYCLE["项目生命周期全流程"]
        ...
        P2["Phase 2: 级联匹配与依赖推断<br>・Auto-Infer: 自动扫 package.json<br>・1st 优先检索: 本地私有冷归档<br>・2nd 熔断保护拉取: Vercel 云端库<br>・首轮自动输出精准来源与 Token 健康卡"]:::phaseStyle
...
1. **第一优先级（本地私有冷库 `skills_archive`）**：
   * 检查您本地经过深度优化、包含 Quality Gate 的私有技能（如 `John-seedance`、`ditther-dark-glass-design`）。如果匹配，直接从本地私有库拷贝至 `项目/.agents/skills/`，汇报卡明确标注：`来源: 本地私有冷库 [skills_archive]`。
2. **第二优先级（Vercel 云端插件库 `vercel-labs/skills`）**：
   * 如果本地私有库没有对应技能（如项目突然需要处理 `Solidity` 合约或 `SvelteKit` 框架），AI 自动调用 Vercel 云端插件 API 去开源社区搜索并拉取最新 Skill，**直接下载到当前项目的 `./.agents/skills/` 临时目录**，汇报卡明确标注：`来源: Vercel 云端 [vercel-labs/skills] | 仅存项目临时目录`。
...
**答**：会的。汇报卡会清晰区分标注：`来源: 本地私有冷库 [skills_archive]` 或 `来源: Vercel 云端 [vercel-labs/skills]`，并提示该云端技能仅保存在项目临时目录中。
...
| **来源追溯** | 技能来源与归属透明 | - [ ] 汇报卡精确标注是来自 `[skills_archive]` 还是 `[vercel-labs/skills]` |
...
- **v2.0.0 (2026-07-28)**：全面实现 v2.0 工业级四大核心模块，新增技能精准来源追溯（区分本地私有冷库与 Vercel 云端源）。
